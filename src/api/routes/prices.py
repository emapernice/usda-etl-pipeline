...
def get_prices(
    ...
    try:
        ...
        total_records = db.execute(
            text(f"SELECT COUNT(*) FROM usda_observations {where_sql}"),
            params
        ).scalar()

        # Fetch data
        sql = text(f"""
            SELECT year, state_name, commodity_desc, price_usd_per_ton
...
def get_prices_by_state(
    ...
    try:
        ...
        total_records = db.execute(
            text("SELECT COUNT(*) FROM usda_observations WHERE state_name = :state"),
            {"state": state.upper()}
        ).scalar()
